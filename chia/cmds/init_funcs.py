...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'metacoin keys generate'")
        return None
...
def chia_init(root_path: Path):
    if os.environ.get("CHIA_ROOT", None) is not None:
        print(
            ...
            f"Please unset the environment variable and run metacoin init again\n"
    ...
    check_keys(root_path)
    print("")
    print("To see your keys, run 'metacoin keys show --show-mnemonic-seed'")
